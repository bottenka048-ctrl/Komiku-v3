...
def cleanup_user_downloads(chat_id):
    ...
    try:
        if chat_id in user_state and isinstance(user_state[chat_id], dict):
            ...
            # Remove chapter folders for decimal chapters
        ...
        if step == "link":
            ...
            try:
                ...
                if '.' in text and '-' not in text:
                    chapter_awal = float(text)
                    chapter_awal_str = text
                elif '-' not in text and not any(c.isalpha() for c in text):
                ...
                # Check if chapter exists in available chapters
                available_chapters = user_state[chat_id].get("available_chapters", [])
                if chapter_awal_str not in available_chapters:
                    bot.reply_to(message, f"❌ Chapter {chapter_awal_str} tidak tersedia.\nChapter tersedia: {', '.join(available_chapters[:10])}...")
                    return

            except ValueError:
                bot.reply_to(message, "❌ Format chapter tidak valid. Contoh: 1, 1.5, 7.2")
                return

            user_state[chat_id]["awal"] = chapter_awal_str
            user_state[chat_id]["step"] = "akhir"
            bot.reply_to(message, f"📌 Masukkan chapter akhir (bisa decimal seperti 7.2):")

        elif step == "akhir":
            try:
                # Check if it's a valid number (int or float)
                if '.' in text and '-' not in text:
                    chapter_akhir = float(text)
                    chapter_akhir_str = text
                elif '-' not in text and not any(c.isalpha() for c in text):
                    chapter_akhir = int(text)
                    chapter_akhir_str = text
                else:
                    bot.reply_to(message, "❌ Format chapter tidak valid. Hindari karakter khusus seperti '-' atau huruf.")
                    return
                ...
                # Check if chapter exists in available chapters
                available_chapters = user_state[chat_id].get("available_chapters", [])
                if chapter_akhir_str not in available_chapters:
            ...
            awal_str = user_state[chat_id].get("awal", "1")
            awal_num = float(awal_str) if '.' in awal_str and '-' not in awal_str else (int(awal_str) if '-' not in awal_str and not any(c.isalpha() for c in awal_str) else float('inf'))
            download_mode = user_state[chat_id].get("mode", "normal")

            if chapter_akhir < awal_num:
                bot.reply_to(message, f"❌ Chapter akhir harus >= chapter awal ({awal_str}).")
                return

            # Calculate chapter count for limits
            available_chapters = user_state[chat_id].get("available_chapters", [])
            awal_index = available_chapters.index(awal_str)
            akhir_index = available_chapters.index(chapter_akhir_str)
            ...
            user_state[chat_id]["akhir"] = chapter_akhir_str
            user_state[chat_id]["step"] = "mode"
            ...
            bot.send_message(chat_id, f"Chapter {awal_str} - {chapter_akhir_str} ({chapter_count} chapter)\nPilih mode download:", reply_markup=markup)
...
def handle_mode(call):
    ...
    try:
        if mode == "gabung":
            all_images = []
            available_chapters = user_state[chat_id].get("available_chapters", [])
            awal_str = user_state[chat_id]["awal"]
            akhir_str = user_state[chat_id]["akhir"]

            # Get the range of chapters to download
            awal_index = available_chapters.index(awal_str)
            akhir_index = available_chapters.index(akhir_str)
