...
def upload_to_drive_and_send_link(chat_id, pdf_path, pdf_name):
    """Upload PDF to Google Drive and send download link to user"""
    try:
        bot.send_message(chat_id, "☁️ Mengupload ke Google Drive...")
        
        # Upload to Google Drive
        result = drive_uploader.upload_file(pdf_path, pdf_name)
        
        if result:
            # Send both shareable and download links
            link_message = (
                f"✅ **{pdf_name}** berhasil diupload ke Google Drive!\n\n"
                f"📎 **Link Download**: {result['download_link']}\n"
                f"🔗 **Link Share**: {result['shareable_link']}\n\n"
                f"💡 Gunakan link download untuk download langsung atau link share untuk preview di browser."
            )
            
            # Create inline keyboard with links
            markup = types.InlineKeyboardMarkup()
            btn_download = types.InlineKeyboardButton("⬇️ Download", url=result['download_link'])
            btn_share = types.InlineKeyboardButton("👀 Preview", url=result['shareable_link'])
            markup.add(btn_download, btn_share)
            
            bot.send_message(chat_id, link_message, reply_markup=markup, parse_mode='Markdown')
            return True
        else:
            bot.send_message(chat_id, "❌ Gagal mengupload ke Google Drive. File akan dikirim langsung.")
            return False
            
    except Exception as e:
        print(f"❌ Google Drive upload error: {e}")
        bot.send_message(chat_id, f"❌ Error upload Google Drive: {e}\nFile akan dikirim langsung.")
        return False
...
def manga_mode(message):
    ...
    tutorial = (
        ...
        "   • GOOGLE DRIVE = upload ke cloud (unlimited size)\n\n"
...
def handle_message(message):
    try:
        ...
        if step == "link":
            ...
            btn_gdrive_gabung = types.InlineKeyboardButton("☁️ Gabung + Google Drive", callback_data="gdrive_gabung")
            btn_gdrive_pisah = types.InlineKeyboardButton("☁️ Pisah + Google Drive", callback_data="gdrive_pisah")
...
@bot.callback_query_handler(func=lambda call: call.data in ["gabung", "pisah", "gdrive_gabung", "gdrive_pisah"])
def handle_mode(call):
    ...
    mode = call.data
    use_google_drive = mode.startswith("gdrive_")
    actual_mode = mode.replace("gdrive_", "") if use_google_drive else mode
        ...
        if actual_mode == "gabung":
            ...
            if all_images and not user_cancel.get(chat_id):
                ...
                try:
                    ...
                    if use_google_drive:
                        # Always use Google Drive when requested
                        upload_success = upload_to_drive_and_send_link(chat_id, pdf_path, pdf_name)
                        if not upload_success:
                            # Fallback to direct upload if Google Drive fails and file is small enough
                            if file_size <= max_size:
                                with open(pdf_path, "rb") as pdf_file:
                                    bot.send_document(
                                        chat_id,
                        ...
                        if file_size > max_size:
                            ...
                            # Suggest Google Drive for large files
                            bot.send_message(chat_id, f"❌ File {pdf_name} terlalu besar ({size_mb:.1f}MB). Limit Telegram adalah 50MB.\n💡 Gunakan opsi 'Google Drive' untuk file besar atau kurangi jumlah chapter.")
                            auto_delete_pdf(pdf_path, 5)
                            return
                    ...
                    if "too large" in error_msg.lower() or "file too big" in error_msg.lower():
                        bot.send_message(chat_id, f"❌ File {pdf_name} terlalu besar untuk Telegram. 💡 Coba gunakan opsi Google Drive.")
                ...
                if imgs:
                    ...
                    try:
                        ...
                        if use_google_drive:
                            ...
                            if file_size > max_size:
                                size_mb = file_size / (1024 * 1024)
                                bot.send_message(chat_id, f"❌ Chapter {ch_str} terlalu besar ({size_mb:.1f}MB). 💡 Gunakan opsi Google Drive untuk file besar.")
                                auto_delete_pdf(pdf_path, 5)
                                continue
                        ...
                        error_msg = str(upload_error)
                        if "too large" in error_msg.lower():
                            bot.send_message(chat_id, f"❌ Chapter {ch_str} terlalu besar untuk Telegram. 💡 Coba gunakan opsi Google Drive.")
                        elif "timeout" in error_msg.lower():
